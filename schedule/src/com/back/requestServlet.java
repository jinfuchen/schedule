...
public class requestServlet extends HttpServlet {
	...
	public void doPost(HttpServletRequest request, HttpServletResponse response)
		...
		//insert request to mysql
		Request patientrequest = new Request();
		patientrequest.setName(name);
		patientrequest.setDate(date);
		patientrequest.setGender(gender);
		patientrequest.setLanguage(language);
		patientrequest.setService(Arrays.toString(services));
		patientrequest.setTime(Arrays.toString(timesection));
		patientrequest.setCity(city);
		//0 represent need to schedule
		patientrequest.setProcess(0);
		//insert the patient request
		RequestDao requestdao = new RequestDao();
		requestdao.InsertRequest(patientrequest);
		//schedule the patient requests
		scheduleRequest schedulereq = new scheduleRequest();
		schedulereq.scheduleProcess(0);
		//redirect the url
		response.sendRedirect("patientReqhistory.jsp");
		out.flush();
		out.close();
	}
